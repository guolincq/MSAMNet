...
class DNANet(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256],deep_supervision=True):
        ...
        if self.deep_supervision:
            ...
            self.final  = nn.Conv2d (nb_filter[0], num_classes+1, kernel_size=1)
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            return [output1[:, -1, :, :].unsqueeze(1), output2[:, -1, :, :].unsqueeze(1), output3[:, -1, :, :].unsqueeze(1), output4[:, -1, :, :].unsqueeze(1)]
        else:
